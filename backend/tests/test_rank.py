...
#higher novelty should select the opposite
...
# Artist diversity cap tests-- verified that radiohead returns radiohead (no novelty bonus)
...
def test_artist_diversity_cap_and_sort_order():
    ...
    assert len(result.candidates) == 2
    assert result.candidates[0].title == "RH0"  # highest Radiohead score (and overall)
...
def test_dropped_seeds_forwarded():
    dropped = ["Artist/Title", "Other/Track"]
    result = rank([], dropped, _params())
    assert result.dropped_seeds == dropped


def test_dropped_seeds_empty_when_none_dropped():
    result = rank([], [], _params())
...
def test_score_is_bounded_0_1():
    # Final score must be 0-1 for all valid inputs since the formula is relevance/novelty blend
    cands = [
